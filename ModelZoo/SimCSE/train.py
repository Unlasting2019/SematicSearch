...
if __name__ == '__main__':
    ...
    #### 读取数据 ####
    start = time.time()
    dataset = load_data('../../data/train_data', sup)
    a_token, b_token, lable = convert_to_ids(dataset, tokenizer, maxlen)
    train_dataset = data_generator(a_token+b_token, batch_size).to_dataset(
        types='int64',
        shapes=[None, maxlen],
        padded_batch=False,
    ...
    with tf.distribute.MirroredStrategy().scope():
        ...
        encoder.summary()
        encoder.compile(loss=simcse_loss, optimizer=Adam(1.5e-5))
        bert.load_weights_from_checkpoint(f'{ptm_dir}/bert_model.ckpt') 

    #### 训练模型 ####
    encoder.fit(
        train_dataset,
        steps_per_epoch=len(a_token)*2 // (batch_size * gpu_num),
        epochs=num_epochs,
        callbacks=[Evaluator()]
    )
    end = time.time()
